...
// Get and increment level by username
app.put('/api/levelUp', async (req, res) => {
  ...
  try {
    // Find the user first to get their current level
    ...
    // Now increment their level by 1
    const currentLevel = user.levelsCompleted;
    ...
    res.status(200).json({
      ...
      newLevel: updatedUser.levelsCompleted,
      user: updatedUser
    });
  } catch (err) {
