...
function Level2Game() {
  ...
  const [timeLeft, setTimeLeft] = useState(60); // Reduced time for Level 2
  ...
  const [obstacleSpeed, setObstacleSpeed] = useState(1); // Increased speed for Level 2
  ...
  useEffect(() => {
    if (gameOver && wordsCompleted === level2Words.length && !levelUpdated) {
  ...
  useEffect(() => {
    if (timeLeft > 0 && !gameOver && !allWordsCompleted) {
      const timer = setInterval(() => {
        ...
        setObstacleSpeed(prev => Math.min(prev + 0.005, 1.5)); // Faster speed increase for Level 2
      }, 1000);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      currentIndex++;
    }, 2000); // Faster obstacle spawn rate for Level 2
  ...
  const handleWordMatch = () => {
    setJumping(true);
    const basePoints = 10; // Increased points for Level 2
  ...
  const levelPassed = wordsCompleted === level2Words.length;
          ...
          <div style={{
            ...
            {levelPassed ? (
              ...
              <p style={{ color: "#ff4444" }}>
                Try again! You need to complete all {level2Words.length} words to pass the level.
              </p>
            )}
