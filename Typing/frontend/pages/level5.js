...
function Level5Game() {
  ...
  const [timeLeft, setTimeLeft] = useState(45); // Time for Level 5
  ...
  const [obstacleSpeed, setObstacleSpeed] = useState(1.3); // Adjusted speed for Level 5
  ...
  useEffect(() => {
    if (gameOver && wordsCompleted === level5Words.length && !levelUpdated) {
  ...
  useEffect(() => {
    if (timeLeft > 0 && !gameOver && !allWordsCompleted) {
      const timer = setInterval(() => {
        ...
        setObstacleSpeed(prev => Math.min(prev + 0.01, 2.0)); // Adjusted speed increase for Level 5
      }, 1000);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      currentIndex++;
    }, 2000); // Adjusted obstacle spawn rate for Level 5
  ...
  const handleWordMatch = () => {
    setJumping(true);
    const basePoints = currentWord.length === 5 ? 10 : 8; // Adjusted points for Level 5
  ...
  const levelPassed = wordsCompleted === level5Words.length;
          ...
          <div style={{
            ...
            {levelPassed ? (
              ...
              <p style={{ color: "#ff4444" }}>
                Try again! You need to complete all {level5Words.length} words to pass the level.
              </p>
            )}
