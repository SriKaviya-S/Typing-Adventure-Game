...
function Level1Game() {
  ...
  const [timeLeft, setTimeLeft] = useState(60); // Increased time for Level 1
  ...
  const [obstacleSpeed, setObstacleSpeed] = useState(1); // Slower initial speed for Level 1
  ...
  useEffect(() => {
    if (gameOver && wordsCompleted === level1Words.length && !levelUpdated) {
  ...
  useEffect(() => {
    if (timeLeft > 0 && !gameOver && !allWordsCompleted) {
      const timer = setInterval(() => {
        ...
        setObstacleSpeed(prev => Math.min(prev + 0.01, 2.0)); // Slower speed increase for Level 1
      }, 1000);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      currentIndex++;
    }, 2000); // Slower obstacle spawn rate for Level 1
  ...
  const handleWordMatch = () => {
    setJumping(true);
    const basePoints = 10; // Reduced points for Level 1
          ...
          <div style={{
            ...
            {levelPassed ? (
              ...
              <p style={{ color: "#ff4444" }}>
                Try again! You need to complete all {level1Words.length} words to pass the level.
              </p>
            )}
