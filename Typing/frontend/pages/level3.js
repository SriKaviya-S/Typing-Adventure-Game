...
function Level3Game() {
  ...
  const [timeLeft, setTimeLeft] = useState(60); // Increased time for more words
  ...
  const [obstacleSpeed, setObstacleSpeed] = useState(1.0); // Slower initial speed for Level 3
  ...
  useEffect(() => {
    if (gameOver && wordsCompleted === level3Words.length && !levelUpdated) {
  ...
  useEffect(() => {
    if (timeLeft > 0 && !gameOver && !allWordsCompleted) {
      const timer = setInterval(() => {
        ...
        setObstacleSpeed(prev => Math.min(prev + 0.01, 2.0)); // Slower speed increase for Level 3
      }, 1000);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      currentIndex++;
    }, 2000); // Slower obstacle spawn rate for Level 3
  ...
  const handleWordMatch = () => {
    setJumping(true);
    const basePoints = currentWord.length === 3 ? 10 : 10; // Points for Level 3
      ...
      <style>
        {`
          @keyframes shake {
            ...
            75% { transform: translateX(- transform: translateX(-10px); }
