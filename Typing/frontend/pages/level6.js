...
function Level6Game() {
  ...
  const [timeLeft, setTimeLeft] = useState(45); // Time for Level 6
  ...
  const [obstacleSpeed, setObstacleSpeed] = useState(1.3); // Adjusted speed for Level 6
  ...
  useEffect(() => {
    if (gameOver && wordsCompleted === level6Words.length && !levelUpdated) {
  ...
  useEffect(() => {
    if (timeLeft > 0 && !gameOver && !allWordsCompleted) {
      const timer = setInterval(() => {
        ...
        setObstacleSpeed(prev => Math.min(prev + 0.01, 2.0)); // Adjusted speed increase for Level 6
      }, 1000);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      currentIndex++;
    }, 1800); // Adjusted obstacle spawn rate for Level 6
  ...
  const handleWordMatch = () => {
    setJumping(true);
    const basePoints = currentWord.length === 5 ? 15 : 10; // Adjusted points for Level 6
  ...
  const levelPassed = wordsCompleted === level6Words.length;
          ...
          <div style={{
            ...
            {levelPassed ? (
              ...
              <p style={{ color: "#ff4444" }}>
                Try again! You need to complete all {level6Words.length} words to pass the level.
              </p>
            )}
