...
function Level8Game() {
  ...
  const [timeLeft, setTimeLeft] = useState(45); // Reduced time for increased difficulty
  ...
  const [obstacleSpeed, setObstacleSpeed] = useState(1.3); // Increased initial speed for Level 8
  ...
  useEffect(() => {
    if (gameOver && wordsCompleted === level8Words.length && !levelUpdated) {
  ...
  useEffect(() => {
    if (timeLeft > 0 && !gameOver && !allWordsCompleted) {
      const timer = setInterval(() => {
        ...
        setObstacleSpeed(prev => Math.min(prev + 0.01, 2.0)); // Faster speed increase for Level 8
      }, 1000);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      currentIndex++;
    }, 1400); // Faster obstacle spawn rate for Level 8
  ...
  const handleWordMatch = () => {
    setJumping(true);
    const basePoints = currentWord.length === 5 ? 25 : 20; // Increased points for Level 8
